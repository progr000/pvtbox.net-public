...
class elFinder {
    ...
    protected $commands = array(
        'getLink'             => array('target' => true),
        'shareDialog'         => array('target' => true),
        'collaborationDialog' => array('target' => true),
        'fileversionsDialog'  => array('target' => true),
        'restoreDeletedFile'  => array('target' => true),
        'share'		          => array('target' => true, 'share_ttl' => false, 'share_lifetime' => false, 'share_password' => false, 'only_change_share_settings' => false),
        'unshare'	          => array('target' => true),
        'hashForPath'         => array('name' => false),

        'open'      => array('target' => false, 'tree' => false, 'init' => false, 'mimes' => false, 'compare' => false, 'showdeleted' => false, 'timezone_offset_seconds' => false),
        'ls'        => array('target' => true, 'mimes' => false, 'intersect' => false),
        'tree'      => array('target' => true),
        'parents'   => array('target' => true, 'showdeleted' => false, 'timezone_offset_seconds' => false),
        'tmb'       => array('targets' => true),
        'file'      => array('target' => true, 'download' => false),
        'zipdl'     => array('targets' => true, 'download' => false),
        'size'      => array('targets' => true, 'ids' => false),
    ...
    const ERROR_SHARE = 'errShare';
    const ERROR_METHOD_NOT_IMPLEMENTED = 'errMethodNotImplemented';
    ...
    {
        //var_dump($this->volumes); exit;
        $target = $args['target'];
        $volume = $this->volumes['l1_'];
        if (method_exists($volume, 'getLink')) {
            return $volume->getLink($target);
        } else {
            return array('error' => $this->error(self::ERROR_METHOD_NOT_IMPLEMENTED, '#'.$target, self::ERROR_METHOD_NOT_IMPLEMENTED));
        }
    }
     ...
     * @return array
     */
    protected function shareDialog($args)
    {
        $target = $args['target'];
        $volume = $this->volume($target);
        if (method_exists($volume, 'shareDialog')) {
            return $volume->shareDialog($target);
        } else {
            return array('error' => $this->error(self::ERROR_SHARE, '#'.$target, self::ERROR_METHOD_NOT_IMPLEMENTED));
        }
    }
     ...
     * @return array
     */
    protected function fileversionsDialog($args)
    {
        $target = $args['target'];
        $volume = $this->volume($target);
        if (method_exists($volume, 'fileversionsDialog')) {
            return $volume->fileversionsDialog($target);
        } else {
            return array('error' => $this->error(self::ERROR_SHARE, '#'.$target, self::ERROR_METHOD_NOT_IMPLEMENTED));
        }
    }
     ...
     * @return array
     */
    protected function restoreDeletedFile($args)
    {
        $target = $args['target'];
        $volume = $this->volume($target);
        if (method_exists($volume, 'restoreDeletedFile')) {
            return $volume->restoreDeletedFile($target);
        } else {
            return array('error' => $this->error(self::ERROR_SHARE, '#'.$target, self::ERROR_METHOD_NOT_IMPLEMENTED));
        }
    }
     ...
     * @return array
     */
    protected function collaborationDialog($args)
    {
        $target = $args['target'];
        $volume = $this->volume($target);
        if (method_exists($volume, 'collaborationDialog')) {
            return $volume->collaborationDialog($target);
        } else {
            return array('error' => $this->error(self::ERROR_SHARE, '#'.$target, self::ERROR_METHOD_NOT_IMPLEMENTED));
        }
    }
     ...
     * @return array
     */
    protected function share($args)
    {
        $target   = $args['target'];
        $share_ttl      = isset($args['share_ttl'])      ? $args['share_ttl'] : null;
        $share_lifetime = isset($args['share_lifetime']) ? $args['share_lifetime'] : null;
        $share_password = isset($args['share_password']) ? $args['share_password'] : null;
        $only_change_share_settings = !empty($args['only_change_share_settings']) ? true : false;
        $model = new \frontend\models\forms\ShareElementForm();
        //var_dump(['ShareElementForm' => $args]);
        //var_dump($model->load(['ShareElementForm' => $args], 'ShareElementForm')); exit;
        if ($model->load(['ShareElementForm' => $args], 'ShareElementForm') && $model->validate()) {
            $volume = $this->volume($target);
            if (method_exists($volume, 'share')) {
                return $volume->share($target, $share_ttl, $share_lifetime, $share_password, $only_change_share_settings);
            } else {
                return array('error' => $this->error(self::ERROR_SHARE, '#' . $target, self::ERROR_METHOD_NOT_IMPLEMENTED));
            }
        } else {
            //var_dump($model->load(['ShareElementForm' => $args], 'ShareElementForm'));
     ...
     * @return array
     */
    protected function unshare($args)
    {
        $target = $args['target'];
        $volume = $this->volume($target);
        if (method_exists($volume, 'unshare')) {
            return $volume->unshare($target);
        } else {
            return array('error' => $this->error(self::ERROR_SHARE, '#'.$target, self::ERROR_METHOD_NOT_IMPLEMENTED));
        }
    }
     ...
     * @return string
     */
    protected function hashForPath($args)
    {
        $name = basename($args['name']);
        $volume = $this->volume('l1_Lw');
        $path = $volume->getPath('l1_Lw');
        return ['link' => Url::to(['/user/files#elf_' . $volume->getHash($path, $name)], CREATE_ABSOLUTE_URL)];
    }
    ...
    protected function open($args) {
        if (isset($args['timezone_offset_seconds'])) {
            $model = new SetTimeZoneOffsetForm();
            if ($model->load(['SetTimeZoneOffsetForm' => $args], 'SetTimeZoneOffsetForm') && $model->validate()) {
                $model->setDynamicTimeZone();
            }
        }
        if (isset($_GET['reload'])) { Yii::$app->session->set('fm_randomizer', md5(microtime())); }
        $target = $args['target'];
        $showdeleted = (isset($args['showdeleted']) && $args['showdeleted'] == 1);
        ...
        foreach ($files as $fk=>$fv) {
            FileSys::setRealFileName($files[$fk]);
        }
        */
        //var_dump($files); exit;
        $fi = 1;
        foreach ($files as $fk=>$fv) {
            if ($files[$fk]['file_deleted'] && !$showdeleted) {
                unset($files[$fk]);
                continue;
            }
            $files[$fk]['num_pp'] = $fi;
        ...
        if (isset($result['cwd']['file_id'], $result['cwd']['file_deleted']) && !$result['cwd']['file_deleted']) {
            Yii::$app->session->set('current_id_dir', $result['cwd']['file_id']);
        } else {
            Yii::$app->session->set('current_id_dir', null);
        }
    ...
    protected function parents($args) {
        ...
        if (($volume = $this->volume($target)) == false
        || ($tree = $volume->parents($target, false, $showdeleted)) == false) {
            return array('error' => $this->error(self::ERROR_OPEN, '#'.$target));
        }
    ...
    protected function size($args) {
        ...
        /* Подсчет размера папки и количества файлов в ней через ФС */
        $size = 0;
        $count = 0;
        $count_files = 0;
        $count_folders = 0;
        ...
        foreach ($args['targets'] as $target) {
            ...
            $ret = $volume->size($target);
            $size += $ret['size'];
            $count += $ret['count_files'];
            $count_files += isset($ret['count_files']) ? $ret['count_files'] : 0;
            $count_folders += isset($ret['count_folders']) ? $ret['count_folders'] : 0;
            $count_children += isset($ret['count_children']) ? $ret['count_children'] : 0;
        }
        return [
            'size' => $size,
            'count' => $count,
        ...
        /* Подсчет размера папки и количества файлов в ней через PostgreSQL */
        //return UserFiles::get_size_and_count_for($args['ids']);
    }
    ...
    protected function mkdir($args) {
        ...
        if ($dirs) {
            ...
            return ($dir = $volume->mkdir($target, $name)) == false
                ? array('error' => $this->error(self::ERROR_MKDIR, $name, $volume->error()))
                : array('added' => array($dir), 'event_data' => $volume::$event_data);
        }
    }
    ...
    protected function mkfile($args) {
        ...
        return ($file = $volume->mkfile($target, $args['name'])) == false
            ...
            : array('added' => array($file), 'event_data' => $volume::$event_data);
    }
    ...
    protected function rename($args) {
        ...
        return ($file = $volume->rename($target, $name)) == false
            ...
            : array('added' => array($file), 'removed' => array($rm), 'event_data' => $volume::$event_data);
    }
    ...
    protected function rm($args) {
        ...
        foreach ($targets as $target) {
            ...
            $result['service_data'] = $volume::$service_data;
            $result['event_data'] = $volume::$event_data;
        }
...
//		}
    ...
    protected function duplicate($args) {
        ...
        foreach ($targets as $target) {
            ...
            //var_dump($src); var_dump($volume);exit;
            if ($src['is_folder']) {
                if (($file = $volume->duplicate_folder($target, $src)) == false) {
                    $result['warning'] = $this->error($volume->error());
                    break;
                }
            } else {
            ...
            //FileSys::setRealFileName($file);
            $result['added'][] = $file;
            $result['event_data'] = $volume::$event_data;
            $result['time'] = Yii::getLogger()->getElapsedTime();
        }
    ...
    protected function paste($args) {
        ...
        foreach ($targets as $target) {
            ...
            if (($src = $srcVolume->file($target)) == false) {
                $result['warning'] = $this->error(self::ERROR_COPY, '#' . $target, self::ERROR_FILE_NOT_FOUND);
                break;
            }
            if ($src['is_folder'] && !$cut) {

                if (($file = $srcVolume->duplicate_folder($target, $src, $dst)) == false) {
        ...
        $result['event_data'] = $dstVolume::$event_data;
        return $result;
    }
