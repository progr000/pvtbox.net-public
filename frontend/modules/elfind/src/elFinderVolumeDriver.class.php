...
abstract class elFinderVolumeDriver {
    ...
    public static $event_data = array();
    public static $service_data = array();
    /**
     * Request args
    ...
    protected $options = array(
        ...
        'tmbPathMode'     => UserFiles::CHMOD_DIR,
    ...
    public function mount(array $opts) {
        ...
        // Normarize disabled (array_mergefor type array of JSON)
     ...
     * @author Dmitry (dio) Levashov
     **/
    public function parents($hash, $lineal = false, $showdeleted = false) {
    ...
    public function size($hash) {
        ...
        if (isset($meta['file_id'])) {
            return UserFiles::countSizeByBD($meta['file_id']);
        } else {
            return UserFiles::countSizeByFS($this->decode($hash));
        }
        //return $this->countSizeByFS($this->decode($hash));
    ...
    public function getLink($target)
    {
        ...
        $UserFile = UserFiles::findOne(['file_id' => $target, 'user_id' => $user_id]);
        ...
        $realpath = realpath($User->_full_path . DIRECTORY_SEPARATOR . $relativePath);

        //var_dump($realpath); exit;
        if (file_exists($realpath)) {
            return [
                'status' => true,
                'url'   => '/user/files#elf_'.$this->encode($realpath),
            ];
        } else {
            return [
                'status' => false,
     ...
     * @return mixed
     */
    public function shareDialog($target)
    {
        $realpath = $this->realpath($target);
        if (file_exists($realpath)) {
            return $this->shareDialogCE($realpath);
        } else {
            return $this->setError(elFinder::ERROR_FILE_NOT_FOUND, $realpath);
        }
    }
     ...
     * @return mixed
     */
    public function fileversionsDialog($target)
    {
        $realpath = $this->realpath($target);
        if (file_exists($realpath)) {
            return $this->fileversionsDialogCE($realpath);
        } else {
            return $this->setError(elFinder::ERROR_FILE_NOT_FOUND, $realpath);
        }
    }
    ...
    /**
     * restoreDeletedFile for file or directory
     *
     * @param string $target
     * @return mixed
     */
    public function restoreDeletedFile($target)
    {
        $realpath = $this->realpath($target);
        if (file_exists($realpath)) {
            return $this->restoreDeletedFileCE($realpath);
        } else {
            return $this->setError(elFinder::ERROR_FILE_NOT_FOUND, $realpath);
        }
    }
     ...
     * @return mixed
     */
    public function collaborationDialog($target)
    {
        $realpath = $this->realpath($target);
        if (file_exists($realpath)) {
            return $this->collaborationDialogCE($realpath);
        } else {
            return $this->setError(elFinder::ERROR_FILE_NOT_FOUND, $realpath);
        }
    }
     ...
     * @return mixed
     */
    public function share($target, $share_ttl=null, $share_lifetime=null, $share_password=null, $only_change_share_settings=false)
    {
        $realpath = $this->realpath($target);
        if (file_exists($realpath)) {
            return $this->shareCE($realpath, $share_ttl, $share_lifetime, $share_password, $only_change_share_settings);
        } else {
            return $this->setError(elFinder::ERROR_FILE_NOT_FOUND, $realpath);
        }
    }
     ...
     * @return mixed
     */
    public function unshare($target)
    {
        $realpath = $this->realpath($target);
        if (file_exists($realpath)) {
            return $this->unshareCE($realpath);
        } else {
            return $this->setError(elFinder::ERROR_FILE_NOT_FOUND, $realpath);
        }
    }
    ...
    /**
     * Create folder copy with suffix "copy number" and return its info
     *
     * @param string $hash
     * @param array $src
     * @return array|bool|false
     */
    public function duplicate_folder($hash, $src, $targetDirHash=null)
    {
        if (isset($src['file_id'], $src['file_parent_id'])) {
            ...
            if ($targetDirHash) {
                $dirParent = $this->decode($targetDirHash);
                $hashParent = $targetDirHash;
                $srcParent = $this->file($hashParent);
            } else {
                $dirParent = $this->dirnameCE($path);
                $hashParent = $this->encode($dirParent);
                $srcParent = $this->file($hashParent);
                //var_dump($srcParent); exit;
            }

            $data['last_event_id'] = UserFiles::last_event_id($src['file_id']);
            $data['target_parent_folder_uuid'] = isset($srcParent['file_uuid']) ? $srcParent['file_uuid'] : "";
            $data['source_folder_uuid'] = $src['file_uuid'];
            $append = ' copy ';

            $new_UserFile_file_name = $this->uniqueName($dirParent, $file['name'], $append);
            $test_length = mb_strlen($new_UserFile_file_name);
            //var_dump($new_UserFile_file_name);
            //var_dump($test_length); exit;
            if ($test_length > UserFiles::FILE_NAME_MAX_LENGTH) {

                do {
                    $file['name'] = mb_substr(
                        $file['name'],
                        $test_length - UserFiles::FILE_NAME_MAX_LENGTH);
                    $new_UserFile_file_name = $this->uniqueName($dirParent, $file['name'], $append);
                    $test_length = mb_strlen($new_UserFile_file_name);
                } while ($test_length > UserFiles::FILE_NAME_MAX_LENGTH);

                $data['target_folder_name'] = $new_UserFile_file_name;
            } else {
                $data['target_folder_name'] = $this->uniqueName($dirParent, $file['name'], $append);
            }
            ...
            if (!$model->load(['NodeApi' => $data]) || !$model->validate()) {
                return $this->setError(self::getErrTxt($model->getErrors()));
                /*
            ...
            $res = $model->folder_event_copy($UserNode, true);

            $created_dir = $dirParent . DIRECTORY_SEPARATOR . $data['target_folder_name'];
            if ($res['result'] === 'success') {
                if (!file_exists($created_dir)) {
                    return $this->setError('Copy folder error');
                }
                return $this->stat($created_dir);
            } elseif ($res['result'] === 'queued') {
                //var_dump(file_exists($created_dir)); exit;
                if (!file_exists($created_dir)) {
                    FileSys::mkdir($created_dir, UserFiles::CHMOD_DIR, true);
                    //mkdir($created_dir);
                    $copying_in_progress = $created_dir . DIRECTORY_SEPARATOR . UserFiles::DIR_COPYING_IN_PROGRESS;
                    touch($copying_in_progress);
                }
                return $this->stat($created_dir);
                //return $this->setError('Copy folder in progress');
            } elseif ($res['result'] === 'error') {
                return $this->setError(self::getErrTxt($res['info']));
            }
        }
        return false;
    }
     ...
     * @return string
     */
    protected static function getErrTxt($err)
    {
        if (is_array($err)) {
            $err_txt = '';
            foreach ($err as $v) {
    ...
    protected function encode($path) {
        if ($path !== '') {
            ...
            // TO-DO crypt path and return hash
            $hash = $this->crypt($p);
        ...
        //TO-DO: Add return statement here
    }
    ...
    protected function decode($hash) {
        if (strpos($hash, $this->id) === 0) {
            ...
            // TO-DO uncrypt hash and return path
            $path = $this->uncrypt($h);
    ...
    protected function stat($path) {
        ...
        if (!$is_root) {
            if (!empty($ret)) {
                ...
                //if ($ret['mime'] !== 'directory') { $ret['mime'] = FileSys::file_mime($path); }

                /** OO */
                $ret['oo_link'] = null;
                if (in_array(mb_strtolower($ret['extension']), Yii::$app->params['online_office_ext'])) {

                    $currentUser = Users::getPathNodeFS(Yii::$app->user->identity->getId());
                    if ($currentUser->license_type == Licenses::TYPE_PAYED_BUSINESS_ADMIN &&
                        $ret['cbl_is_owner'] &&
                        $currentUser->user_oo_address)
                    {

                        $ret['oo_link'] = $currentUser->user_oo_address . '/oo/' . $ret['file_uuid'];

                    } elseif ($currentUser->license_type == Licenses::TYPE_PAYED_BUSINESS_USER &&
                              isset($ret['collaboration_id']))
                    {
                        $collaborationOwner = UserCollaborations::getOwner($ret['collaboration_id']);
                        $ownerUser = Users::findIdentity($collaborationOwner);
                        if ($collaborationOwner != $currentUser->user_id && $ownerUser->user_oo_address) {

                            $ret['oo_link'] = $ownerUser->user_oo_address . '/oo/' . $ret['file_uuid'];
        ...
        if (isset($ret['file_lastmtime']) && $ret['file_lastmtime'] > 0) { $ret['ts'] = $ret['file_lastmtime']; }
        if (isset($ret['file_size']) && ($ret['file_size'])) { $ret['size'] = $ret['file_size']; }
    ...
    protected function updateCache($path, $stat) {
        ...
        // @to-do move dateformat to client
     ...
     * @return array
     */
    protected function countSizeByFS($path, &$ret=array()) {
        ...
        if (file_exists($path) && !is_dir($path)) {
            //var_dump($path);
            if (FileSys::basename($path) !== UserFiles::DIR_INFO_FILE) {
                $info = UserFiles::getFileShareInfo($path);
                //var_dump($info);
                if (!$info['file_deleted']) { $ret['size'] += intval($info['file_size']); }
                if (!$info['file_deleted']) { $ret['count']++; }
            }
        }
        ...
        if (is_dir($path)) {
            if ($h = @opendir($path)) {
                while (false !== ($f = readdir($h))) {
                    ...
                    if (!file_exists($path . DIRECTORY_SEPARATOR . UserFiles::DIR_INFO_FILE))
                        continue;

                    $info2 = UserFiles::getFileShareInfo($path);
                    if ($info2['file_deleted'])
                        continue;

                    if (($f === '.') || ($f === '..') || ($f === UserFiles::DIR_INFO_FILE))
                        continue;

                    $newpath = $path . DIRECTORY_SEPARATOR . $f;
                    $this->countSizeByFS($newpath, $ret);
    ...
    **/
    ...
    public function rmdirRecursive($dir) {
        if (!is_link($dir) && is_dir($dir)) {
            ...
            if ($handle = opendir($dir)) {
                while (false !== ($file = readdir($handle))) {
                    ...
                    if (!is_link($dir) && is_dir($path)) {
                        ...
                        chmod($path, UserFiles::CHMOD_FILE);
                        unlink($path);
                    }
                }
                closedir($handle);
            ...
            chmod($dir, UserFiles::CHMOD_FILE);
            return unlink($dir);
        }
        return false;
    }
     ...
     * @return mixed
     */
    abstract protected function shareDialogCE($path);

    /**
     * Метод получения ноды-файлменеджера
     *
     * @return \common\models\UserNode
     */
    abstract protected function getUserNode();
     ...
     * @return mixed
     */
    abstract protected function fileversionsDialogCE($path);
     ...
     * @return mixed
     */
    abstract protected function restoreDeletedFileCE($path);
     ...
     * @return mixed
     */
    abstract protected function collaborationDialogCE($path);
     ...
     * @return mixed
     */
    abstract protected function shareCE($path, $share_ttl, $share_lifetime, $share_password, $only_change_share_settings);
     ...
     * @return mixed
     */
    abstract protected function unshareCE($path);
     ...
     *
     * @param  string  $path  file path
     * @param  string  $mode  octal string such as '0 7 5 5'
     * @return bool
     * @author David Bartle,
     **/
