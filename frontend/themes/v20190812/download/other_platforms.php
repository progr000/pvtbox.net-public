...
<ul class="platforms-download" id="other-platforms" style="display: none;">
    ...
    foreach ($software as $k => $v) {
        ...
        ?>
        <li><!--
        --><a class="btn xs-btn <?= (in_array($v['software_type'], [Software::TYPE_ANDROID, Software::TYPE_IOS])) ? $v['software_type'] : "primary" ?>-btn download-btn"
              data-version="<?= $v['software_version'] ?>"
              data-software-type="<?= $v['software_type'] ?>"
              target="<?= (in_array($v['software_type'], [Software::TYPE_ANDROID, Software::TYPE_IOS])) ? "_blank" : "_self" ?>"
              rel="noopener"
              id="download-<?= $v['software_type'] ?>"
              title="<?= $title ?>"
              href="<?= $v['software_url'] ?>"><?= (in_array($v['software_type'], [Software::TYPE_ANDROID, Software::TYPE_IOS])) ? "" : Yii::t('app/download', 'download') ?></a><!--
        --><span><?= Software::getType($v['software_type'], true) ?> <?= $v['software_description'] ?></span><!--
     --></li>
        <?php
    }
    ?>
