 ...
 * jslint: loopfunc: true, browser: true, ass: true, bitwise: true, continue: true, nomen: true, plusplus: true, regexp: true, unparam: true, to-do: true, white: true
 */
/*jshint -W083 */
	...
	$.jstree.core.prototype = {
		...
		bind : function () {
			...
			this.element
				.on("dblclick.jstree", function (e) {
					...
					if (typeof $(e.target).attr('data-file-uuid') !== 'undefined') {
						AdminPanelAddNewFolderForColleague($(e.target).attr('data-file-uuid'));
					}
					return false;
		...
		_parse_model_from_flat_json : function (d, p, ps) {
			...
			var tid = d.id.toString(),
				...
				tmp = {
					...
					text		: d.text || '',
					icon		: d.icon !== undefined ? d.icon : true,
					file_uuid	: d.file_uuid !== undefined ? d.file_uuid : "",
					parent		: p,
					parents		: ps,
					children	: d.children || [],
					children_d	: d.children_d || [],
		...
		draw_children : function (node) {
			...
			if(!node || !node.length) { return false; } // TO-DO: quick toggle
		...
		redraw_node : function (node, deep, is_callback, force_render) {
			...
			node.id = obj.id;
			//console_log($(node).attr('id'));
			//console_log(obj);
			$(node).attr('data-file-uuid', obj.file_uuid);
			node.className = c;
			...
			$(node.childNodes[1]).attr('data-file-uuid', obj.file_uuid);
			...
			$(node.childNodes[1].childNodes[0]).attr('data-file-uuid', obj.file_uuid);
 ...
 */
	...
	var drg, elm;
	// TO-DO: now check works by checking for each node individually, how about max_children, unique, etc?
