...
/**
 * Если на странице есть елемент с ИД = SignUrl
...
$(document).ready(function() {
    if ($('#SignUrl').length) {
        ...
        ws = new Ws($('#SignUrl').text(), 5); // второй параметр если 0 то реконекта не будет, иначе реконект после заданного кол-ва секунд
            ...
            ws.onclose = function () {
                ...
                if (ws.reconnect > 0) {
                    setTimeout(function () {
                        ...
                        if ($('#SignUrl').attr('data-token').length) {
                            $.ajax({
                                type: 'post',
                                url: _LANG_URL + '/user/check-site-token',
                                data: {
                                    site_token: $('#SignUrl').attr('data-token'),
                                },
                                dataType: 'json',
                            ...
                            console_log('Disconnected from ' + ws._url);
                            console_log("#SignUrl hasn't attribute data-token. Can't reconnect.");
                        }
