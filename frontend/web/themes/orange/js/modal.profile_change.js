$(document).ready(function() {
    ...
    $(document).on('click', '.admin-panel-select-folder', function () {
        if (checkNodesOnline(false)) {
            $('#folder-select-modal').modal({"show": true});
        }
    });
    ...
    /** */
    $('#folder-select-modal').on('shown.bs.modal', function (e) {
        $('#available-folder-list').niceScroll({cursorcolor:"#CDCDCD", cursorwidth:"8px"});
    })

    /** */
    $('#folder-select-modal').on('hidden.bs.modal, hide.bs.modal', function () {
        $("#available-folder-list").getNiceScroll().remove();
    });
});
