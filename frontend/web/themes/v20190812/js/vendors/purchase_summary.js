$(document).ready(function() {
    ...
    /** */
    $(document).on('change', '#pbm-os1', function() {
        $('#pba-os1').val( $(this).val() );
    });

    /** */
    $(document).on('change', '#pba-os1', function() {
        $('#pbm-os1').val( $(this).val() );
    });

    /** */
    $(document).on('change', '#pbm-os2', function() {
        $('#pba-os2').val( $(this).val() );
    });

    /** */
    $(document).on('change', '#pba-os2', function() {
        $('#pbm-os2').val( $(this).val() );
    });

    /** */
    $(document).on('change', '#pbm-os0', function() {
        //alert($('option:selected', this).data('value'));
        checkIsLicenseLessThanBefore( parseInt($('option:selected', this).data('value')) );
    ...
    $(document).on('change', '#server-license-count-monthly', function() {
        //alert($(this).val());
        checkIsServerLicenseLessThanBefore( parseInt($(this).val()) );
    ...
    /** */
    if (typeof count_licenses_total != 'undefined' && count_server_licenses_total != 'undefined') {
        //$("#server-license-count-monthly [value='" + count_server_licenses_total + "']").attr("selected", "selected");
        //$("#server-license-count-annually [value='" + count_server_licenses_total + "']").attr("selected", "selected");
        $('#server-license-count-monthly').val(count_server_licenses_total);
        $('#server-license-count-annually').val(count_server_licenses_total);
        initSelectFields();
        recalculateSaveSum();

        setTimeout(function() {
            //$("#pbm-os0 [value='" + count_server_licenses_total + '-' + count_licenses_total + "']").attr("selected", "selected");
            //$("#pba-os0 [value='" + count_server_licenses_total + '-' + count_licenses_total + "']").attr("selected", "selected");
            $('#pbm-os0').val(count_server_licenses_total + '-' + count_licenses_total);
            $('#pba-os0').val(count_server_licenses_total + '-' + count_licenses_total);
            initSelectFields();
            recalculateSaveSum();
        }, 100);
    }
 ...
 * @param count_buy_now
 */
function checkIsServerLicenseLessThanBefore(count_buy_now)
{
    if (typeof count_server_licenses_used != 'undefined') {
        if (count_server_licenses_used > count_buy_now) {
            var $text_container = $('#alert-text-for-less-server-licenses-than-before');
            if ($text_container.length) {
                prettyAlert($text_container.text());
            }
        }
