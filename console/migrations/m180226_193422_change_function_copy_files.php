...
/**
 * Class m180226_193422_change_function_copy_files
 */
class m180226_193422_change_function_copy_files extends Migration
{
    /**
     * {@inheritdoc}
     */
    public function safeUp()
    {
        ...
        $this->db->pdo->exec("
            ...
            DROP FUNCTION IF EXISTS copy_files(bigint, bigint, character varying, character varying, bigint, boolean);

            CREATE OR REPLACE FUNCTION copy_files(
                id bigint,                               -- ид папки которую будем копировать
                parent_id bigint,                        -- ид родительской папки куда будет копиоваться папка
                new_name character varying,              -- новое имя для копируемой папки
                separator character varying,             -- фс сепаратор
                max_timestamp bigint,                    -- таймстам события когда было вызвано копирование
                event_uuid_from_node character varying,  -- event_uuid для копируемой папки (только верхний уровень, остальные ууиды будут сгенерированы)
                _is_debug boolean)                       -- флаг отладки. если труе то не будут созданы записи в бд
            ...
            ALTER FUNCTION copy_files(bigint, bigint, character varying, character varying, bigint, character varying, boolean)
                OWNER TO {$userName};
        ");
    }
    ...
    {
        echo "m180226_193422_change_function_copy_files cannot be reverted.\n";
