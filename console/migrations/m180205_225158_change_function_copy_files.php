...
/**
 * Class m180205_225158_change_function_copy_files
 */
class m180205_225158_change_function_copy_files extends Migration
{
    /**
     * @inheritdoc
     */
    public function safeUp()
    {
        ...
        Yii::$app->db->pdo->exec("
            ...
            DROP FUNCTION IF EXISTS copy_files(bigint, bigint, character varying, character varying);
            ...
            begin
                    ...
                    WITH RECURSIVE obj_tree as (
                        SELECT * FROM (
                            SELECT DISTINCT ON (e1.file_id)
                                ...
                                e1.*
                            FROM {$tablePrefix}user_files as f1
                            INNER JOIN {$tablePrefix}user_file_events as e1 ON f1.file_id = e1.file_id
                    ...
                    UNION ALL
                        SELECT * FROM (
                            SELECT distinct on (e2.file_id)
                                ...
                                md5(text(uuid_generate_v4())) as new_uuid,
                                e2.*
                            FROM {$tablePrefix}user_files as f2
                            INNER JOIN obj_tree ff ON ff.file_id = f2.file_parent_id
                            INNER JOIN {$tablePrefix}user_file_events e2 ON f2.file_id = e2.file_id
                ...
                loop
                    ...
                    if _rec.is_folder = 1 then
                        ...
                        _rec.file_hash = null;
                        _rec.file_name_before_event = _file.file_name_before_event; --@@-- event
                        _rec.file_name_after_event = _file.file_name_after_event; --@@-- event
                        ...
                        _rec.file_hash = _file.file_hash; --@@-- event
                        _rec.file_name_before_event = _file.file_name_before_event; --@@-- event
                        _rec.file_name_after_event = _file.file_name_after_event; --@@-- event
                        _rec.file_size_before_event = _file.file_size_before_event; --@@-- event
                        _rec.file_size_after_event = _file.file_size_after_event; --@@-- event
            ...
            ALTER FUNCTION copy_files(bigint, bigint, character varying, character varying, bigint)
            OWNER TO {$userName};
        ");
    }
    ...
    {
        echo "m180205_225158_change_function_copy_files cannot be reverted.\n";
