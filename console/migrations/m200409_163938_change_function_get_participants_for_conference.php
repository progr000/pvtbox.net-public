...
/**
 * Class m200409_163938_change_function_get_participants_for_conference
 */
class m200409_163938_change_function_get_participants_for_conference extends Migration
{
    /**
     * {@inheritdoc}
     */
                    ...
                    RETURN QUERY (
                        ...
                        FROM (
                            ...
                            SELECT
                                ...
                                (CASE WHEN t1.user_id = $1 THEN t3.user_email ELSE t1.colleague_email END) AS participant_email,
                                NULL AS conference_id,
                                (CASE WHEN t1.user_id = $1 THEN t2.user_id ELSE t1.user_id END) AS user_id,
                            ...
                            WHERE (/*где он владелец колабы*/(t2.user_id = $1) AND /*и его коллеги*/((t1.user_id != $1) OR (t1.user_id IS NULL)))
                            OR (/*где он участник колабы*/(t2.user_id != $1) AND (t1.user_id = $1) AND (t1.colleague_status = 'joined'))
    ...
    {
        echo "m200409_163938_change_function_get_participants_for_conference cannot be reverted.\n";
