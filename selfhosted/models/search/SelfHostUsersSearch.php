<?php

namespace selfhosted\models\search;

use yii\base\Model;
use yii\data\ActiveDataProvider;
use common\models\SelfHostUsers;

/**
 * AdminsSearch represents the model behind the search form of `backend\models\Admins`.
 */
class SelfHostUsersSearch extends SelfHostUsers
{
    /**
     * {@inheritdoc}
     */
    public function rules()
    {
        return [
            [['shu_id', 'shu_status', 'shu_role'], 'integer'],
            [['shu_name', 'shu_email', 'auth_key', 'password_hash', 'password_reset_token', 'shu_created', 'shu_updated'], 'safe'],
        ];
    }

    /**
     * {@inheritdoc}
     */
    public function scenarios()
    {
        // bypass scenarios() implementation in the parent class
        return Model::scenarios();
    }

    /**
     * Creates data provider instance with search query applied
     *
     * @param array $params
     *
     * @return ActiveDataProvider
     */
    public function search($params)
    {
        $query = self::find();

        // add conditions that should always apply here

        $dataProvider = new ActiveDataProvider([
            'query' => $query,
        ]);

        $this->load($params);

        if (!$this->validate()) {
            // uncomment the following line if you do not want to return any records when validation fails
            // $query->where('0=1');
            return $dataProvider;
        }

        // grid filtering conditions
        $query->andFilterWhere([
            'shu_id' => $this->shu_id,
            'shu_created' => $this->shu_created,
            'shu_updated' => $this->shu_updated,
            'shu_status' => $this->shu_status,
            'shu_role' => $this->shu_role,
        ]);

        $query->andFilterWhere(['ilike', 'shu_name', $this->shu_name])
            ->andFilterWhere(['ilike', 'shu_email', $this->shu_email])
            ->andFilterWhere(['ilike', 'auth_key', $this->auth_key])
            ->andFilterWhere(['ilike', 'password_hash', $this->password_hash])
            ->andFilterWhere(['ilike', 'password_reset_token', $this->password_reset_token]);

        return $dataProvider;
    }
}
